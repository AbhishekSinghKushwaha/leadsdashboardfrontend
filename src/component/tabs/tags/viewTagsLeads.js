...
const TagLeads = (props) => {
    ...
    const csvheaders = [
        ...
        { label: 'Type', key: 'type' },
        { label: 'No of Positions', key: 'noofpositions' },
        { label: 'Experience', key: 'experience' },
        { label: 'SourceURL', key: 'sourceurl' },
        { label: 'Contact Name', key: 'userdetails[0].contactname' },
        { label: 'Email', key: 'userdetails[0].emailaddress' },
        { label: 'Phone', key: 'userdetails[0].phone' },
        { label: 'Designation', key: 'userdetails[0].designation' },
        { label: 'Last Reply', key: 'lastreply' },
        { label: 'Status', key: 'status' },
        { label: 'When to Connect', key: 'whentoconnect' },
        { label: 'Comments', key: 'comments' },
        { label: 'Lastfollowup', key: 'lastfollowup' },
    ...
    const getUser = (tagName) => {
      ...
      useEffect(() => {
        getUser(props.match.params.tagName);
        dispatch(fetchAll());
        ...
        <span className="pfooter">
            <div className="row">
                <div className="column">
                    <h6 className="totalleads">Total Leads: {leads.length}</h6>
                </div>
                <div className="column">
                    <Pagination currentPage={currentPage} postsPerPage={postsPerPage} totalPosts={leads.length} paginate={paginate}/>
                </div>
            </div>
        </span>
    );
    ...
    return ( 
        <div className="display-details">
            ...
            <div className="view-title">
                ...
                {google?(<>
                <div>
                <Button label="Add Lead" icon="pi pi-plus" className="p-button-sm p-button-info p-button-outlined" onClick={openNew} style={{marginRight: '.5em'}}/>

                    <CSVLink data={leads} headers={csvheaders} filename={"Leads_Data_"+props.match.params.tagName+".csv"} className="export">
                    <Button label="Export" icon="pi pi-download" className="p-button-sm p-button-secondary p-button-outlined">
                    </Button>
                    </CSVLink>
            ...
            <Dialog visible={userDialog} style={{ width: '600px' }} header={user._id? "Update Leads Details" : "Add Leads Details"} modal className="p-fluid" footer={userDialogFooter} onHide={hideDialog}>
